...
\subsubsection{Arquitetura de Sistemas de Informação}
...
\clearpage
\subsubsection{Arquitetura de Tecnologia}
...
\begin{figure}[htbp]
\centering
\begin{tcolorbox}[enhanced, colback=white, colframe=gray!40, arc=3mm, boxrule=0.5pt, width=0.85\textwidth]
\begin{tabular}{|p{12cm}|}
\hline
\rowcolor{green!10}\large \textbf{Forças (Strengths)} \\
\hline
\begin{itemize}\setlength{\itemsep}{1pt}
\item A implementação do ERP pode aumentar a eficiência, reduzir erros e otimizar os processos de segurança do trabalho, impulsionando a modernização da instituição e justificando investimentos futuros.

\item A viabilidade técnica do projeto é favorecida pelo escopo focado em segurança do trabalho, facilitado pela regulamentação clara.

\item O apoio da alta gestão garante recursos, facilita a adoção do sistema e incentiva a equipe a aceitar as mudanças, aumentando as chances de sucesso na implementação do ERP.

\item O IFPE utiliza o sistema SEI, que, apesar de alguns processos manuais e descentralizados, oferece recursos tecnológicos adequados.

\item A equipe de implantação demonstra alto engajamento e disponibilidade, concentrando esforços na execução do projeto com excelência e adaptando-se bem às necessidades específicas.
\end{itemize} \\
\hline
\end{tabular}
\end{tcolorbox}
...
\hline
\begin{itemize}\setlength{\itemsep}{1pt}
\item A instituição enfrenta dificuldades para lidar com mudanças, o que pode gerar resistência à adoção do novo sistema e exigir mais tempo para treinamentos e adaptação.

\item A rigidez dos processos e a resistência dos servidores tornam a implementação mais complexa e demorada.

\item Por estar fortemente inserido na rotina da equipe de segurança do SIASS, o processo pode encontrar resistência por parte dos usuários.

\item A existência de vários sistemas temporários dificulta a integração, aumenta a complexidade operacional e pode gerar retrabalho durante a transição para o novo ERP.

\item A migração de dados representa um gargalo, exigindo rigorosos controles e validação.
\end{itemize} \\
\hline
\end{tabular}
\end{tcolorbox}
